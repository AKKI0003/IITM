...
function LightboxIcon({ path, className = "w-6 h-6" }) {
  return (
...
function Lightbox({ index, onClose, onNext, onPrev }) {
  ...
  return (
    <div
      ...
      className="fixed inset-0 z-[100] flex items-center justify-center bg-ink-navy/95 backdrop-blur-sm animate-[fadeIn_0.2s_ease-out]"
      onClick={onClose}
    >
      <button
        type="button"
      ...
      >
        <LightboxIcon path="M6 6l12 12M18 6L6 18" />
      </button>
      ...
      >
        <LightboxIcon path="M15 6l-6 6 6 6" />
      </button>
      ...
      >
        <LightboxIcon path="M9 6l6 6-6 6" />
      </button>

      <div
        className="relative max-w-[92vw] sm:max-w-[86vw] lg:max-w-[70vw] max-h-[86vh] flex flex-col items-center"
      ...
      >
        <img
          ...
          src={item.image}
          alt={item.label}
          className="max-h-[68vh] w-auto object-contain rounded-md shadow-2xl animate-[scaleIn_0.2s_ease-out]"
        />
        <div className="mt-4 text-center px-4">
          <span className="inline-block font-mono text-[0.68rem] tracking-[0.14em] uppercase text-civic-amber font-semibold mb-1">
            {item.category}
          </span>
          <h3 className="text-paper font-display font-semibold text-[1.15rem] leading-snug">{item.label}</h3>
...
export default function Gallery() {
  ...
  return (
    <section id="gallery" className="py-16 lg:py-section bg-mist">
          ...
          {items.map((g, i) => (
            ...
            >
              ...
              <span className="absolute top-3 right-3 inline-flex items-center justify-center w-8 h-8 rounded-full bg-white/15 backdrop-blur-sm text-white opacity-0 group-hover:opacity-100 transition-opacity duration-200">
                <LightboxIcon path="M9 3H3v6M15 3h6v6M15 21h6v-6M9 21H3v-6" className="w-4 h-4" />
              </span>
            </button>
          ))}
        </div>
      ...
      {activeIndex !== null && <Lightbox index={activeIndex} onClose={close} onNext={next} onPrev={prev} />}
    </section>
  );
}
